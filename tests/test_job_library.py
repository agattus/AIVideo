...
def _seed_run_dir(run_dir: Path, *, title: str = "Old Film", idea: str = "A myth story") -> None:
...
def test_list_jobs_from_memory_store() -> None:
    fake = _FakeRedis()
    ...
    jobs = list_jobs(limit=10, client=fake)  # type: ignore[arg-type]
...
def test_list_jobs_discovers_output_folders(tmp_path: Path, monkeypatch) -> None:
    ...
    fake = _FakeRedis()
    jobs = list_jobs(limit=20, client=fake)  # type: ignore[arg-type]
...
def test_get_jobs_and_reopen_endpoints(tmp_path: Path, monkeypatch) -> None:
    ...
    with (
        patch(
            ...
            side_effect=lambda limit=40: list_jobs(limit=limit, client=fake),  # type: ignore[arg-type]
        ),
        patch(
