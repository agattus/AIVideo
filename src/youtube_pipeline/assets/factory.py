...
from youtube_pipeline.assets.stock import PexelsProvider, PixabayProvider
...
def build_asset_provider(settings: Settings | None = None) -> AssetProviderProtocol:
    ...
    if settings.asset_provider == AssetProvider.PEXELS:
        return PexelsProvider(settings)
    if settings.asset_provider == AssetProvider.PIXABAY:
        return PixabayProvider(settings)
